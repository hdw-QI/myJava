...
<html>
<head>
    <title>前端md5加密加验案例</title>
    <link rel="stylesheet" href="./css/layui.css">
</head>
<body>
<style>
    .demo-reg-container{width: 500px; height: 400px; margin: 200px auto 0;}
    ...
    <div class="demo-reg-container">
        ...
        <div class="layui-form-item">
            <div class="layui-input-wrap">
                ...
                <input type="password" name="confirmPassword" value="" lay-verify="required|confirmPassword" placeholder="确认密码" autocomplete="off" class="layui-input" id="confirm-password" lay-affix="eye">
            </div>
        </div>
...
<script>
    layui.use(function(){
        ...
        form.on('submit(demo-reg)', function(data){
            ...
            // 加密加盐
            let salt='qqdw';
            let md5Pwd=md5(field.password,salt)
            // $('#reg-password').val(md5Pwd)
            // $('#confirm-password').val(md5Pwd)
            field.password=md5Pwd
            field.confirmPassword=md5Pwd
            const stringify = JSON.stringify(field);
            // 显示填写结果，仅作演示用
            // layer.alert(stringify, {
            //     title: '当前填写的字段值'
            // });

            // 此处可执行 Ajax 等操作
            $.post('md5/userRegister',stringify,(res)=>{
                // console.log(res)
                if (res.data){
                    window.location.href='md5Login.jsp'
                }
            })
